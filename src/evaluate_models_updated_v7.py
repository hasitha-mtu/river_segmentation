...
VALID_SPLITS = {'sequential', 'stratified', 'alternative'}

@dataclass
class CheckpointInfo:
    path    : Path
    run_id  : str   # e.g. '512_1', '512_2'
    seed    : str   # e.g. '42', '123'
    split   : str   # 'sequential' | 'stratified' | 'alternative'


def discover_checkpoints(experiments_dir: str,
                         resolution: str = '512_2') -> list[CheckpointInfo]:
    """
    Walk experiments_dir / resolution  and collect all best.pth files,
    excluding wandb mirror copies.

    Expected path layout:
        {experiments_dir}/{resolution}/{seed}/{split}/.../{model}/checkpoints/best.pth

    Returns CheckpointInfo objects sorted by (seed, split, path) for
    reproducibility.
    """
    base = Path(experiments_dir) / resolution
    if not base.exists():
        raise FileNotFoundError(
            f'Resolution directory not found: {base}\n'
            f'Available: {[d.name for d in Path(experiments_dir).iterdir() if d.is_dir()]}'
        )

    infos: list[CheckpointInfo] = []
    for p in base.rglob('checkpoints/best.pth'):
        if 'wandb' in p.parts:
            continue

        # Parse seed and split from the path relative to base
        # Expected: base / {seed} / {split} / ...
        rel_parts = p.relative_to(base).parts  # ('42', 'stratified', 'unet', ...)
        if len(rel_parts) < 3:
            print(f'  [SKIP] Cannot parse seed/split from path: {p}')
            continue
        ...
        if split not in VALID_SPLITS:
            print(f'  [SKIP] Unrecognised split "{split}" in: {p}')
            continue

        # sam_v1_fine_tuned is excluded from this benchmark
        if 'sam_v1_fine_tuned' in p.parts:
            continue
    ...
    infos.sort(key=lambda c: (c.seed, c.split, str(c.path)))

    print(f'\nDiscovered {len(infos)} checkpoint(s) under: {base}')
    for c in infos:
        print(f'  [{c.seed}|{c.split}]  {c.path.relative_to(base)}')
    return infos
...
def model_family(config: dict) -> str:
    ...
    if name in ('convnext_upernet', 'hrnet_ocr'):
        return 'Hybrid SOTA'
    if name in ('sam', 'sam_fpn', 'sam_v2_fine_tuned', 'dinov2', 'dinov2_Mask2Former'):
        return 'Foundation'
    return 'Other'
...
def _forward_dinov2_Mask2Former(model  : nn.Module,
    batch  : dict,
    device : torch.device,
    config : dict,
    predictor  : SAM2ImagePredictor = None):
    ...
    if model.training:
        ...
        seg_logits, m2f_loss = model(
                pixel_values=images,
                mask_labels=mask_labels,
                class_labels=class_labels,
            )
        return seg_logits, masks
    else:
...
def evaluate_model(
    ...
    for batch in tqdm(loader, desc='  Inference', leave=False, ncols=90):
        ...
        if model_name == 'sam':
            ...
            if outputs is None:
                continue  # all items in batch were invalid — skip silently
            per_image, acc = _per_image_metrics_sam2(per_image, acc, timing, outputs, masks, batch, device, t0,
                                                     threshold)
        elif model_name == 'dinov2_Mask2Former':
            outputs, masks = _forward_dinov2_Mask2Former(model, batch, device, config, predictor)
            per_image, acc = _per_image_metrics(per_image, acc, timing, outputs, batch, device, t0, threshold)
        else:
...
def _per_image_metrics_sam2(per_image, acc, timing, outputs, masks,
                            batch, device, t0, threshold):
    # outputs: [B, 1, H, W] logits   masks: [B, H, W] or [B, 1, H, W]
    probs = torch.sigmoid(outputs)  # [B, 1, H, W]

    # Ensure masks are [B, 1, H, W] for MetricAccumulator
    if masks.dim() == 3:
...
def _per_image_metrics(per_image, acc, timing, outputs, batch, device,
                       t0, threshold):
...
# ─────────────────────────────────────────────────────────────────────────────
def save_prediction_overlays_sam_v2_fine_tuned(
        model         : nn.Module,
        loader        : DataLoader,
        device        : torch.device,
        out_dir       : Path,
...
def parse_args():
    p = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent('''\
            Examples:
              # Primary multi-seed evaluation on 512_1 (seeds 42, 123, 456 × stratified):
              python evaluate_models_updated_v7.py --resolution 512_1 --splits stratified

              # All splits for seed 42 only:
              python evaluate_models_updated_v7.py --resolution 512_1 --seeds 42

              # Cross-check: independent 512_2 seed 42 run:
              python evaluate_models_updated_v7.py --resolution 512_2 --seeds 42 --splits stratified

              # Resolution sensitivity — 768:
              python evaluate_models_updated_v7.py --resolution 768 --seeds 42

              # Resolution sensitivity — 1024:
              python evaluate_models_updated_v7.py --resolution 1024 --seeds 42
        '''),
    )
    ...
    # Per-resolution dataset roots.  The script derives the correct one
    # automatically from --resolution; these flags allow overrides if needed.
    _dataset_base = r'C:/Users/StudentAdikariAdikar/Projects/river_segmentation/dataset'
    p.add_argument('--data_root_512',
                   default=f'{_dataset_base}/processed_512_resized',
                   help='Dataset root for 512-resolution experiments (512_1 and 512_2).')
    p.add_argument('--data_root_768',
                   default=f'{_dataset_base}/processed_768_resized',
                   help='Dataset root for 768-resolution experiments.')
    p.add_argument('--data_root_1024',
                   default=f'{_dataset_base}/processed_1024_resized',
                   help='Dataset root for 1024-resolution experiments.')

    p.add_argument('--resolution', default='512_1',
                   help='Resolution sub-directory to evaluate (512_1, 512_2, 768, 1024).')
    p.add_argument('--splits', nargs='+',
                   default=['sequential', 'stratified', 'alternative'],
    ...
    p.add_argument('--seeds', nargs='+', default=None,
                   help='Restrict to specific seed(s) (e.g. --seeds 42 123). Default: all seeds found.')
...
def _resolve_data_root_base(args) -> str:
    ...
    r = args.resolution
    if r in ('512_1', '512_2'):
        return args.data_root_512
    if r == '768':
        return args.data_root_768
    if r == '1024':
        return args.data_root_1024
    # Unknown resolution: fall back to 512 and warn
    print(f'[WARN] Unrecognised resolution "{r}" — defaulting to 512 dataset root.')
    return args.data_root_512
...
def main():
    ...
    # Resolve the correct dataset root for this resolution
    data_root_base = _resolve_data_root_base(args)
    ...
    print(f'Data root base  : {data_root_base}')
    ...
    all_ckpts = discover_checkpoints(args.experiments_dir, resolution=args.resolution)

    # Apply seed and split filters
    ckpt_infos = [
        c for c in all_ckpts
        if c.split in args.splits
        and (args.seeds is None or c.seed in args.seeds)
    ]
    if not ckpt_infos:
        print('\n[ERROR] No best.pth checkpoints found after filtering.')
        print('Make sure training has completed and the experiments directory is correct.')
        sys.exit(1)

    print(f'\nAfter filtering: {len(ckpt_infos)} checkpoint(s) to evaluate.')

    output_dir = Path(args.output_dir) / args.resolution
    output_dir.mkdir(parents=True, exist_ok=True)

    # Accumulate results grouped by (seed, split) so we can write one CSV per
    # group AND one combined CSV across all groups.
    ...
    for ckpt_info in ckpt_infos:
        ...
        split     = ckpt_info.split
        seed      = ckpt_info.seed
        run_id    = ckpt_info.run_id

        # Dataset directory for this split — always from data_root_base.
        # The trained model's embedded config['data']['data_root'] may point to
        # the training machine's path; we ignore it and use the canonical path
        # derived from the split name instead.
        split_data_root = Path(data_root_base) / split

        print(f'\n{"─" * 80}')
        print(f'Checkpoint : {ckpt_path}')
        print(f'Run / Seed / Split : {run_id} | {seed} | {split}')
        print(f'Dataset root       : {split_data_root}')
        ...
        try:
            ...
            failed_models.append(f'{display_name} [{seed}|{split}]')
            continue
        ...
        if args.save_predictions:
            pred_dir = output_dir / 'predictions' / f'seed_{seed}' / split / display_name
            ...
            try:
                if model_name == 'sam_v2_fine_tuned':
                    save_prediction_overlays_sam_v2_fine_tuned(
                        ...
                        out_dir=pred_dir, n_samples=args.num_pred_samples,
                        threshold=args.threshold,
                    )
                else:
                    save_prediction_overlays(
                        model=model, loader=test_loader, device=device,
                        out_dir=pred_dir, is_sam=is_sam,
                        n_samples=args.num_pred_samples, threshold=args.threshold,
                    )
            except Exception as e:
    ...
    # Combined CSV across all seeds and splits
    run_tag = args.resolution
    save_csv(all_results,   output_dir / f'test_results_{run_tag}.csv')
    save_csv(per_image_all, output_dir / f'test_results_per_image_{run_tag}.csv')

    # Per-(seed, split) CSVs and leaderboards — useful for cross-split analysis
    from itertools import groupby
    keyfn = lambda r: (r['seed'], r['split'])
    for (seed_k, split_k), group in groupby(
            sorted(all_results, key=keyfn), key=keyfn):
        group_list = list(group)
        sub_dir = output_dir / f'seed_{seed_k}' / split_k
        sub_dir.mkdir(parents=True, exist_ok=True)
        save_csv(group_list, sub_dir / 'test_results.csv')

        per_img_group = [
            r for r in per_image_all
            if r['seed'] == seed_k and r['split'] == split_k
        ]
        save_csv(per_img_group, sub_dir / 'test_results_per_image.csv')

        leaderboard_text = print_leaderboard(group_list)
        summary_path = sub_dir / 'test_results_summary.txt'
    ...
    # ── Best model highlight (over primary split: stratified) ─────────────────
    primary = [r for r in all_results if r['split'] == 'stratified'] or all_results
    best = max(primary, key=lambda r: r['dice'])
    print('\n' + '★' * 60)
    print(f'  BEST MODEL  :  {best["model"]}')
    print(f'  Split       :  {best["split"]}  (seed {best["seed"]})')
