"""
SAM Segmentation Decoder
========================

Lightweight decoder for semantic segmentation using SAM encoder features.
...
class FPNDecoder(nn.Module):
    ...
    def __init__(
        ...
        # Top-down pathway (3x3 convs to refine)
    ...
    def forward(
        ...
        Args:
            features: List of features [f1, f2, f3, f4] from encoder
        ...
        for i in range(len(laterals) - 1, -1, -1):
            if i == len(laterals) - 1:
                # Highest level (coarsest)
                fpn_feat = laterals[i]
            else:
                # Add upsampled coarser feature
...
if __name__ == "__main__":
    print("Testing SAM Decoder")
    
    # Simulate encoder features
    features = [
        torch.randn(2, 96, 128, 128),    # 1/4
        torch.randn(2, 192, 64, 64),     # 1/8
        torch.randn(2, 384, 32, 32),     # 1/16
        torch.randn(2, 768, 16, 16),     # 1/32
    ]
