...
class UnifiedTrainer:
    ...
    def __init__(self, config: dict):
        ...
        # ── Optimizer ─────────────────────────────────────────────────────
        # Hybrid SOTA models implement get_params_groups() but must NOT use
        # differential LR in this benchmark — see note below.
        _HYBRID_SOTA_MODELS = {'hrnet_ocr', 'convnext_upernet'}
        ...
        elif hasattr(self.model, 'get_params_groups') and model_name not in _HYBRID_SOTA_MODELS:
            # Foundation / transformer models (DINOv2, SegFormer, etc.) expose
            # get_params_groups() for differential per-group learning rates.
            #
            # Hybrid SOTA models (hrnet_ocr, convnext_upernet) also implement
            # get_params_groups() but are EXCLUDED here.  Their get_params_groups()
            # sets the pretrained backbone at 0.1× LR, which effectively freezes it
            # while the randomly-initialised segmentation head tries to learn from
            # static features — causing catastrophic non-convergence when combined
            # with boundary loss.  They use flat LR (matching their previous run)
    ...
    def _compute_loss(self, main_out, masks, aux_out=None):
        ...
        For Hybrid SOTA models (HRNet-OCR, ConvNeXt-UPerNet) aux_out is a plain
        Tensor.  We pass it directly to CombinedLoss via the aux_predictions
        parameter so that boundary loss and aux weighting are handled in one
        place.  This avoids calling self.criterion twice and keeps the train/val
        loss on the same scale (previously, calling criterion twice in train mode
        while eval-mode models return a single tensor inflated train loss ~1.4×).
        ...
        """
        is_combined = self.config['loss']['type'] == 'combined'

        # ── Hybrid SOTA: Tensor aux output ────────────────────────────────
        # Pass aux directly into CombinedLoss so boundary loss is applied once
        # (via aux_predictions) rather than via a second criterion() call.
        # For non-combined losses this path is skipped — aux handled below.
        if aux_out is not None and isinstance(aux_out, torch.Tensor) and is_combined:
            total, loss_dict = self.criterion(main_out, masks, aux_out)
        ...
        if aux_out is not None:
            if isinstance(aux_out, torch.Tensor):
                # Non-combined loss with Tensor aux (rare — guard against tuple
                # unpacking that would fail when criterion returns a scalar).
                ...
                # GlobalLocal: aux_out is a tuple (g_logits, l_logits)
...
def main():
    ...
    for dataset_variation in dataset_variations:
        ...
        # default_config['training']['epochs'] = 5
