...
# from src.train_unified_wandb_sam_v1 import get_sam1_dataset
...
def build_model_from_config(config: dict, ckpt: dict, device: torch.device, ckpt_path) -> nn.Module:
    ...
    if model_name == 'sam':
        ...
        print(f'  [SAM] Rebuilding {variant}')
        ...
        model = SamModel.from_pretrained(model_tag)
        model.load_state_dict(ckpt['model_state_dict'])
    else:
        model = get_model(
...
def model_family(config: dict) -> str:
    ...
    if name in ('convnext_upernet', 'hrnet_ocr'):
        return 'Hybrid SOTA'
    if name in ('sam', 'sam_fpn', 'sam_v1_fine_tuned', 'sam_v2_fine_tuned', 'dinov2', 'dinov2_Mask2Former'):
        return 'Foundation'
    return 'Other'
...
@torch.no_grad()
def _forward_sam1_eval(model, batch, device, config, predictor):
...
def evaluate_model(
    ...
    for batch in tqdm(loader, desc='  Inference', leave=False, ncols=90):
        ...
        if model_name == 'sam':
            ...
            outputs, masks = _forward_sam1_eval(model, batch, device, config, predictor)
            per_image, acc = _per_image_metrics_sam1(per_image, acc, timing, outputs, masks, batch, device, t0,
                                                     threshold)
        elif model_name == 'sam_v2_fine_tuned':
...
def _per_image_metrics_sam1(per_image, acc, timing, outputs, masks,
...
# ─────────────────────────────────────────────────────────────────────────────
def save_prediction_overlays_sam_v1_fine_tuned(
        model         : nn.Module,
        loader        : DataLoader,
        device        : torch.device,
        out_dir       : Path,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(min(len(image_full_paths), n_samples - saved)):
                image_full_path = image_full_paths[i]

                orig_h = int(batch['original_sizes'][i][0])
                orig_w = int(batch['original_sizes'][i][1])
                img_pil = Image.open(image_full_path).convert('RGB').resize(
                    (orig_w, orig_h), Image.BILINEAR
                )
                img_np = np.array(img_pil).astype(np.float32) / 255.0

                mask_np = masks[i].cpu().numpy()  # (512, 512)
                pred_np = preds[i].numpy()  # (512, 512)
...
def parse_args():
    p = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent('''\
            Examples:
              # Evaluate all splits for the primary 512x512 run (seed 42 + 123):
              python evaluate_models_updated_v5.py

              # Only the stratified split, seed 42:
              python evaluate_models_updated_v5.py --splits stratified --seeds 42

              # Evaluate the older 512_1 run:
              python evaluate_models_updated_v5.py --resolution 512_1
        '''),
    )
    ...
    p.add_argument('--data_root_base',
                   default=r'C:/Users/AdikariAdikari/PycharmProjects/river_segmentation/dataset/processed_512_resized',
                   help='Parent directory containing sequential/, stratified/, alternative/ sub-dirs.')
    p.add_argument('--resolution', default='512_2',
                   help='Resolution sub-directory to evaluate (e.g. 512_2, 512_1, 768, 1024).')
...
def main():
    ...
    print(f'Data root base  : {args.data_root_base}')
    ...
    for ckpt_info in ckpt_infos:
        ...
        split_data_root = Path(args.data_root_base) / split
        ...
        try:
            ...
            # elif model_name == 'sam_v1_fine_tuned':
            #     test_ds = get_sam1_dataset(
            #         str(split_data_root), config['model']['variant']
            #     )
            #     test_loader = DataLoader(
            #         test_ds,
            #         batch_size=args.batch_size,
        ...
        # Nest overlays under seed/split so runs never overwrite each other.
